...
function calcPeakFlow(area, coeff, rain) {
  // Rational method: Q (m³/s) = C × I (mm/hr) × A (m²) / 3,600,000
  // Design storm intensity = 25 mm/hr (Marathwada/Vidarbha standard)
  // IS:15797:2008 Cl.5.2 | Kiran & Kumar 2023
  const I_design = 25; // mm/hr fixed design storm
  return +(coeff * I_design * area / 3600000).toFixed(6);
}
...
function calcDesignParams(totalArea, tankSize, weightedC, rain) {
  // ── First-flush volume: IS:15797 Clause 5.3
  const firstFlushVol = +(totalArea / 40).toFixed(1); // litres

  // ── Tank dimensions with realistic depth cap
  // Base formula: V = L×B×D at L:B = 2:1
  // But depth capped at 3.5m max for underground RCC (structural & constructability limit)
  // If volume needs more depth, we expand L and B instead — IS:3370 compliance
  const MAX_DEPTH = 3.5; // m — max practical depth for underground RCC tank
  let tankD = MAX_DEPTH;
  // L×B = V/D, at L:B = 2:1 → 2B² = V/D → B = sqrt(V/(2D))
  let tankB = +Math.sqrt(tankSize / (2 * tankD)).toFixed(1);
  let tankL = +(2 * tankB).toFixed(1);
  // If single tank becomes too wide (>20m), recommend splitting — show units
  const tankUnits = tankL > 20 ? Math.ceil(tankSize / 100) : 1;
  if (tankUnits > 1) {
    const singleVol = tankSize / tankUnits;
    tankB = +Math.sqrt(singleVol / (2 * tankD)).toFixed(1);
    tankL = +(2 * tankB).toFixed(1);
  }

  // ── Filter bed — IS:15797 Clause 5.4
  // Max 4 m² per unit (practical field constraint), split into multiple units
  const totalFilterArea = +(Math.max(totalArea / 50, 0.5)).toFixed(2);
  const filterUnitArea  = Math.min(totalFilterArea, 4.0); // m² per unit, max 4
  const filterUnits     = Math.ceil(totalFilterArea / filterUnitArea);
  const filterDia       = +(Math.sqrt(filterUnitArea / Math.PI) * 2).toFixed(2); // dia of each unit

  // ── Pipe sizing via Manning — corrected rational method
  ...
  return {
    firstFlushVol,
    tankL, tankB, tankD, tankUnits,
    totalFilterArea, filterUnitArea, filterUnits, filterDia,
    peakFlow, pipeDia, velocity,
  };
}
      ...
      {activeTab === "overview" && (
        <>
          ...
          <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", gap:"0.75rem", marginBottom:"1rem" }}>
            ...
            <MetricCard label="Downpipe Dia"     icon="🔧" value={`${dp.pipeDia}mm`}        unit="PVC downpipe" color={C.structGrey} sub={`Peak Q = ${(dp.peakFlow*1000).toFixed(2)} L/s · n=0.01`} />
        ...
        <>
          ...
          <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap:"0.85rem", marginBottom:"0.85rem" }}>
            {[
              { title:"RCC Underground Tank", sub:"Storage Tank", bg:C.hydroBlue, rows:[
                ...
                ["No. of Tanks", dp.tankUnits > 1 ? `${dp.tankUnits} units × ${(tankSize/dp.tankUnits).toFixed(0)} m³ each` : "1 tank", dp.tankUnits > 1 ? "Split recommended for constructability" : "Single tank feasible"],
                ["Each Tank L×B×D", `${dp.tankL}m × ${dp.tankB}m × ${dp.tankD}m`, "Depth capped at 3.5m · IS:3370"],
                ["First-Flush Vol", `${dp.firstFlushVol} L`, "Roof area ÷ 40 · IS:15797 Cl.5.3"],
                ["First-Flush Pipe", `${dp.pipeDia}mm PVC`],
              ]},
              { title:"Dual-Media Filter", sub:"Filtration System", bg:C.aquiferTeal, rows:[
                ["Total Filter Area", `${dp.totalFilterArea} m²`, "Roof area ÷ 50 · IS:15797 Cl.5.4"],
                ["No. of Filter Units", `${dp.filterUnits} unit${dp.filterUnits > 1 ? "s" : ""}`, `Max 4 m² per unit — field standard`],
                ["Each Unit Diameter", `∅${dp.filterDia}m`, `${dp.filterUnitArea.toFixed(1)} m² per unit`],
                ["Gravel Layer", "300mm", "20–40mm graded gravel"],
                ["Sand Layer", "300mm", "Fine sand 0.6–2mm · Total 600mm"],
              ]},
              { title:"Gutters & Downpipes", sub:"Conveyance System", bg:C.monsoonBlue, rows:[
                ["Design Intensity", "25 mm/hr", "Marathwada design storm · IS:15797"],
                ["Peak Runoff", `${(dp.peakFlow*1000).toFixed(2)} L/s`, "Q = C×I×A / 3,600,000 (m³/s)"],
                ["Pipe Diameter", `${dp.pipeDia}mm PVC`, "Manning n=0.01, S=1/100"],
                ["Flow Velocity", `${dp.velocity} m/s`, dp.velocity >= 0.6 ? "Self-cleaning ✓" : "⚠ Increase slope"],
                ["Gutter Slope", "1:100 min", "IS:15797 recommendation"],
              ]},
          ...
          <div style={{ background:`${C.hydroBlue}07`, border:`1px dashed ${C.hydroBlue}28`, borderRadius:"9px", padding:"0.9rem 1.1rem" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns: isMobile?"1fr":"1fr 1fr", gap:"0.4rem 2rem", fontSize:"0.73rem", color:C.graphite, lineHeight:2, fontFamily:"'IBM Plex Mono', monospace" }}>
              {["Q = P × A × C × (1−FF) / 1000","S_t = min(max(S_(t-1)+Q_t−D_t,0),Cap)","WSE = Σmet / Σdemand × 100","V_dry = t × n × q × 0.4","Q_peak = C × I(mm/hr) × A / 3,600,000","Manning: V = (1/n) × R^(2/3) × S^(1/2)"].map(eq => <div key={eq}>{eq}</div>)}
            </div>
          </div>
        </>
      )}
      ...
      {activeTab === "summary" && (
        <div style={{ background:C.pureWhite, border:`1px solid ${C.lightBorder}`, borderRadius:"10px", overflow:"hidden", boxShadow:"0 1px 4px rgba(15,76,129,0.06)" }}>
          ...
          {[
            ...
            ["Recommended Tank",`${tankSize} m³`],["Tank Dims",`${dp.tankL}×${dp.tankB}×${dp.tankD}m (D≤3.5m)`],
            ["Tank Units", dp.tankUnits > 1 ? `${dp.tankUnits} tanks` : "1 tank"],
            ["First-Flush Vol",`${dp.firstFlushVol} L`],["Filter Bed",`${dp.totalFilterArea}m² · ${dp.filterUnits} unit${dp.filterUnits>1?"s":""} · ∅${dp.filterDia}m each`],
